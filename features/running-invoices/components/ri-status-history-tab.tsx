import type { RiStatusHistoryRow } from '../server/queries'
...
export function RiStatusHistoryTab({ history }: { history: RiStatusHistoryRow[] }) {
  if (history.length === 0) {
    return <div className={styles.empty} style={{ padding:'32px 0' }}>No history available.</div>
  }
  return (
        ...
        <div key={h.id} className={`${styles.historyItem} ${i === history.length - 1 ? styles.historyLast : ''}`}>
          ...
          <div className={styles.historyContent}>
            <div className={styles.historyTransition}>
              {h.fromStatus && <><span className={styles.historyFrom}>{RI_STATUS_LABELS[h.fromStatus] ?? h.fromStatus}</span> → </>}
              <span className={`${styles.statusBadge} ${styles[`ri_${h.toStatus}` as keyof typeof styles] ?? ''}`}>
                {RI_STATUS_LABELS[h.toStatus] ?? h.toStatus}
              </span>
            </div>
